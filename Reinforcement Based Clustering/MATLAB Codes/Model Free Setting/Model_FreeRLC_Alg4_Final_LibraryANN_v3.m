...
for inner = 1 : size(MaxInner,1)
    ...
    T = 50; Tmin = 0.001; tau1 = 0.9;    % annealing parameters
    ...
    while T >= Tmin
        ...
        idx = cell(K,1);
        for i = 1 : M
            col_i = Pij(i,1)*col(1,:) + Pij(i,2)*col(2,:) + Pij(i,3)*col(3,:) + Pij(i,4)*col(4,:);
            scatter(X(i,1),X(i,2),90,'filled','MarkerEdgeColor','k',...
                'MarkerFaceColor',col_i,'LineWidth',0.15); hold on;
        end
        for k = 1 : K
